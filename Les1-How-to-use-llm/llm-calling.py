...
if __name__=="__main__":
    ...
    FilledPrompt = summary_prompt_template.format(information=information)
    print(f"STEP1 LLMに対するプロンプト: {FilledPrompt}")
    
    # =====STEP2: LLMに投げて結果を得る=====
    LLMOutput = llm.invoke(input=FilledPrompt)
    print(f"STEP2 LLMの応答: {LLMOutput}\n")

    # =====STEP3: LLMの応答を整形=====
    parserdOutput = StrOutputParser().parse(LLMOutput).content
    print(f"STEP3 LLMの応答を整形: {parserdOutput}\n")
